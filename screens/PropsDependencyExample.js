  ...
  return (
    <View style={styles.root}>
      ...
      <View>
        ...
        <Text>{'- This useEffect(..) WORKS AS INTENDED but it will trigger an Eslint error.\n'}</Text>
        <Text>{'- The reason for the error is because the way Javascript functions fundamentally work.\n' +
        ...
        <Text>{'- We can fix the error by destructuring "props" outside useEffect(..).\n'}</Text>
        <Text>{'- We can also choose to ignore the error since we never actually use the passed "props", so there is no risk.'}</Text>
      </View>
    </View>
  );
};
